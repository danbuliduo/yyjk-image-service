...
export const getTableData = async () => {
      ...
      await Promise.all(
        fieldMetas.map(async (fieldMeta) => {
          ...
          if (fieldMeta.type === FieldType.Attachment) {
            console.log('附件字段原始值:', fieldMeta.name, cellValue);
          }
  ...
  const columns = fieldMetas.map((meta) => {
    return {
      title: meta.name,
      dataIndex: meta.id,
      // 获取渲染函数（附件字段的 render 已在 render.tsx 中注册）
  ...
  // 构建 字段ID → 字段元信息 的映射，供 App 层挂载渲染器使用
  const fieldMetaMap = new Map<string, IFieldMeta>();
  fieldMetas.forEach((meta) => fieldMetaMap.set(meta.id, meta));
